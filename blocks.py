...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        self.w = normal.sample((self.out_features, self.in_features))
        self.b = normal.sample((out_features, 1))
        # ========================
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        xwt = torch.mm(x, self.w.t())
        out = torch.add(xwt, self.b.t().expand(x.shape[0], self.b.shape[0]))
        # ========================
...
class ReLU(Block):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.mul((x > 0).to(x.dtype), dout)
        # ========================
...
class Sigmoid(Block):
    ...
    def forward(self, x, **kw):
        ...
        # grad_cache.
        # ====== YOUR CODE: ======
        out = torch.ones_like(x) / (torch.ones_like(x) + torch.exp(-x))
        self.grad_cache['x'] = x
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache['x']
        dx = torch.mul(dout, torch.exp(-x) * torch.pow(torch.ones_like(x) + torch.exp(-x), -2))
        # ========================
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        loss = (-x[range(N), y] + torch.log(torch.exp(x).sum(1))).sum() / N
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        dx = torch.exp(x).div(torch.exp(x).sum(dim=1, keepdim=True))
        dx[range(N), y] -= 1
        dx /= N
        # ========================
